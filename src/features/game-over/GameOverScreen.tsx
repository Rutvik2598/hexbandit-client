...
function Crown({ winnerColor }: { winnerColor: string }) {
  return (
    <svg
...
function VpChip({ chip }: { chip: VpChip }) {
  return (
    <span style={{
      ...
      {chip.icon && (
        <Icon name={chip.icon} size={15} color="var(--text-dim)" />
      )}
      <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-dim)' }}>
        {chip.count != null ? `${chip.count} ` : ''}{chip.label}
      </span>
      <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--amber-soft)' }}>
        +{chip.vp}
      </span>
    </span>
  );
...
function StandingRow({ ranked, humanLost, totalPlayers }: {
  ...
  // Server's victory_points excludes VP dev cards (they're secret); sum breakdown for the true total
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, minWidth: 128, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {markHuman && (
            <span style={{
              fontSize: 9.5, fontWeight: 800, letterSpacing: '0.08em', color,
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexShrink: 0, paddingLeft: 6 }}>
        <span style={{
          ...
          fontSize: isWinner ? 42 : 34, lineHeight: 1,
        ...
        <span style={{
          ...
          color: isWinner ? 'var(--amber-soft)' : 'var(--text-faint)',
        }}>VP</span>
      </div>
    </div>
  );
...
export function GameOverScreen({ gameState, humanPlayerIndices, onPlayAgain, onMenu, onReplay }: Props) {
  ...
  const ranked: RankedPlayer[] = useMemo(() => {
    return [...gameState.players]
      ...
      .sort((a, b) => {
        // Winner always first, then sort by computed VP
  ...
  // If no human in game, treat it as a "win" (celebration) perspective for the spectator
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 50, display: 'grid', placeItems: 'center', overflow: 'auto' }}>
      ...
      <motion.div
        ...
        style={{
          ...
          background: 'rgba(4,8,16,0.65)', backdropFilter: 'blur(8px)',
      ...
      <motion.div
        ...
        style={{ position: 'relative', zIndex: 3, width: 780, maxWidth: '94vw', padding: '44px 0 40px' }}
      >
        {/* winner crest */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 28 }}>
          <span className="eyebrow" style={{
            color: humanLost ? '#8b9bb5' : 'var(--amber-soft)',
            letterSpacing: '0.34em', marginBottom: 16,
          }}>
            {isFullBotGame ? 'Game Over' : humanLost ? 'Defeat' : 'Victory'}
          </span>

          <Crown winnerColor={winnerColor} />

          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 14 }}>
            <span style={{
              width: 16, height: 16, borderRadius: '50%', background: winnerColor,
              boxShadow: `0 0 16px ${winnerColor}`,
            }} />
            <h1 style={{
              fontFamily: 'var(--ff-display)', fontWeight: 700, fontSize: 50, margin: 0, letterSpacing: '0.02em',
          ...
          <p style={{ margin: '10px 0 0', fontSize: 15, fontWeight: 600, color: 'var(--text-dim)' }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 22, justifyContent: 'center' }}>
          <button onClick={onMenu} className="btn" style={{ padding: '14px 26px', fontSize: 14.5, fontWeight: 700, fontFamily: 'var(--ff-display)', letterSpacing: '0.04em' }}>
            Main Menu
          </button>
          <button
            onClick={onReplay}
